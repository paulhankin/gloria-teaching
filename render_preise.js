...
function drawPlant(svg, rc, x, seed) {
  // freistehende Pflanze OHNE Topf: Stiel bis zum Boden, unten kleiner Wurzelballen
  const b = BASE;
  ...
  // Wurzelballen
  svg.appendChild(rc.ellipse(x, b - 4, 78, 30, opts(seed + 10, "#7a5230", "#c9a06e")));
  [[-26, 10], [0, 14], [26, 10]].forEach((r, i) =>
    line(svg, rc, x + r[0] * 0.6, b - 8, x + r[0], b + r[1], opts(seed + 20 + i, "#7a5230", null, 2.5)));
}
...
function drawSandwich(svg, rc, x, seed) {
  ...
  // obere Toastscheibe (flach, wie die untere)
...
function buildPicture(spec, seed) {
  ...
  const x1 = 240, x2 = 660;
  const drawers = {
    pflanze: [() => drawPlant(svg,rc,x1,seed+1), () => drawPot(svg,rc,x2,seed+20)],
